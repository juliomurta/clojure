...
(defn- between-999999-and-1000
  [num]
  (def th-hundred (extract-hundred (quot num 1000)))
  (def th-ten (extract-ten th-hundred))
  (def th-until-20 (extract-unity-if-more-than-20 th-ten))  
  (str (build-string-until-999 th-hundred th-ten th-until-20) " thousand"))

(defn- between-999999999-and-1000000
  [num]
  (def mi-hundred (extract-hundred (quot num 1000000)))
  (def mi-ten (extract-ten mi-hundred))
  (def mi-until-20 (extract-unity-if-more-than-20 mi-ten))  
  (if (and (>= num 1000000) (< num 999999999))    
    (str (build-string-until-999 mi-hundred mi-ten mi-until-20) " million")))

(defn- between-999999999999-and-1000000000
  [num]
  (def bi-hundred (extract-hundred (quot num 1000000000)))
  (def bi-ten (extract-ten bi-hundred))
  (def bi-until-20 (extract-unity-if-more-than-20 bi-ten))  
  (if (and (>= num 1000000000) (< num 999999999999))
    (str (build-string-until-999 bi-hundred bi-ten bi-until-20) " billion")))
